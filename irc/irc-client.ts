...
const botPath = 'src/bot/index.js'
...
function createIRCClient() {
  ...
  irc.on('registered', () => {
    log('[connected]')
    if (nickservPW) irc.say('nickserv', `identify ${nickservPW}`)
  })
...
function createBot() {
  const bot = fork(botPath, [config.server, config.nick])
  ...
  bot.on('message', (message) => {
    ...
    if (command === 'join') irc.join(target)
    if (command === 'part') irc.part(target, '')
    if (command === 'say') irc.say(target, text)
    if (command === 'action') irc.action(target, text)
  })
